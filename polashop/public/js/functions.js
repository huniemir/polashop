...
function ajaxPostFormAppend(id_form,url,id_to_reload,append_name,append_value)
{
    var form = document.getElementById(id_form);
    var formData = new FormData(form); 
    formData.append(append_name, append_value);
    var xmlHttp = new XMLHttpRequest();
	xmlHttp.onreadystatechange = function() {
		if(xmlHttp.readyState == 4 && xmlHttp.status == 200) {
			document.getElementById(id_to_reload).innerHTML = xmlHttp.responseText;
...
function ajaxPostOne(name,value,url,id_to_reload){
    var xmlHttp = new XMLHttpRequest();
	xmlHttp.onreadystatechange = function() {
		if(xmlHttp.readyState == 4 && xmlHttp.status == 200) {
			document.getElementById(id_to_reload).innerHTML = xmlHttp.responseText;
...
function ajaxPostOneAndForm(flag,form,name,value,url,id_to_reload){
	if(flag){
		ajaxPostFormAppend(form,url,id_to_reload,name,value);
	} 
	else{
		ajaxPostOne(name,value,url,id_to_reload);
	} 
...
function addToCart(id,url){
    ...
    xmlHttp.open("POST", url, true); 
    xmlHttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xmlHttp.send("id="+id+"&amount="+amount); 
}
